  ...
  for settings_file in "${settings_files[@]}"; do
    ...
    if [[ $settings_file != $main_settings_file ]]; then
      ...
      for setting in "${settings_commented_out[@]}"; do
        ...
        escaped_setting=$(echo "$setting" | sed -e 's/\./\\./g')
        ...
        # actually confuse it even more.
        # shellcheck disable=SC2181
        if grep -E -n "^[[:space:]]*${escaped_setting}" $settings_file >>/dev/stderr; then
          err $test "Found $setting in $settings_file, but it should be commented out or not present. See above for details."
        else
          echo "$test: $setting is commented out or not present in $settings_file"
        fi
      done
    fi
  done
